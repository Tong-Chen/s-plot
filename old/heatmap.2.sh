...
${txtbld}OPTIONS${txtrst}:
	...
	-u	The width of pciture.[${txtred}Default 800${txtrst}]
	-v	The height of pciture.[${txtred}Default 800${txtrst}]
	-R	The resolution of picture.[${txtred}Default NA.${txtrst}]
...
hv <- heatmap.2(x, col=greenred, trace="${trace}", 
...
hv <- heatmap.2(x, col=greenred(75), trace="${trace}", 
	xlab="$xlab", ylab="$ylab",
	main="$title", margins=c(7,12), keysize=0.6, Rowv=${row_C},
	Colv=${col_C}, dendrogram=c("${dendrogram}"), scale=c("$scale"))
}
