...
This script is used to do contour plot instead of heatmap 
using function ggplot and stat_contour from package ggplot2. 

The parameters for logical variable are either TRUE or FALSE.

${txtbld}OPTIONS${txtrst}:
	-f	Data file (with header line, no column would be treated as 
 		rownames, tab seperated)${bldred}[NECESSARY]${txtrst}
	-m	One string added to the output file name to indicate the
		meaning of this file.
		${bldred}[Optional, no default value, normally is the combination of crucial 
	...
	-c	The variable for levelheight of contour, usually the main
		value column you want to display.${bldred}[NECESSARY]${txtrst}
