...
if ("" != ""){
	data_m$Set <- cut(data_m$Set, )
} else if ("" != ""){
	level_i <- c()
	data_m$Set <- factor(data_m$Set, levels=level_i)
}
if ("" != ""){
	data_m$Set <- cut(data_m$Set,)
}else if ("" != ""){
	x_level <- c()
	data_m$Set <- factor(data_m$Set,levels=x_level)
}
...
p <- ggplot(data_m, aes(x=factor(Set), ymin=minimum, lower=lower_quantile, 
			middle=median, upper=upper_quantile, ymax=maximum)) +
			xlab(" ") + ylab(" ") + labs(title="")


if (TRUE){
	p <- p + geom_boxplot(aes(fill=factor(Set)), notch=TRUE,
		notchwidth=0.3, stat = "identity")
}else {
	p <- p + geom_boxplot(aes(fill=factor(Set)), stat = "identity")
}
...
if ("pdf" == "pdf") {
	ggsave(p, filename="boxplot_precomputed_values.boxplot.value_Set.pdf", dpi=300, width=20,
	height=12, units=c("cm"),colormodel="srgb")
} else {
	ggsave(p, filename="boxplot_precomputed_values.boxplot.value_Set.pdf", dpi=300, width=20,
	height=12, units=c("cm"))
}
#png(filename="boxplot_precomputed_values.boxplot.value_Set.png", width=20, height=12,
#res=300)
#p
#dev.off()
