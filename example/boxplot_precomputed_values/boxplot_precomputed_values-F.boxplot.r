...
data_m <- read.table(file="boxplot_precomputed_values-F", sep="\t", header=TRUE,
...
p <- ggplot(data_m, aes(x=factor(variable), ymin=minimum, lower=lower_quantile, 
...
if ("pdf" == "pdf") {
	ggsave(p, filename="boxplot_precomputed_values-F.boxplot.pdf", dpi=300, width=20,
	height=12, units=c("cm"),colormodel="srgb")
} else {
	ggsave(p, filename="boxplot_precomputed_values-F.boxplot.pdf", dpi=300, width=20,
	height=12, units=c("cm"))
}
#png(filename="boxplot_precomputed_values-F.boxplot.png", width=20, height=12,
#res=300)
#p
#dev.off()
