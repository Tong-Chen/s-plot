...
if(! FALSE){
	ID_var <- c('price','cut','color','clarity')
	data <- read.table(file="diamond.matrix", sep="\t", header=TRUE,
	row.names=1)
	...
	data_m <- read.table(file="diamond.matrix", sep="\t",
	header=TRUE)
}
...
ggsave(p, filename="diamond.matrix.boxplot.png", dpi=300, width=20,
height=12, units=c("cm"))

#png(filename="diamond.matrix.boxplot.png", width=20, height=12,
#res=300)
#p
#dev.off()
